var oldRateLimitTimer = new Date().getTime();
var rateLimitTimer = new Date().getTime();
var passGiven;
var clicker = 0;
var oldPassGiven;
var urlString;
...
function timedScheduler() {
    rateLimitTimer = new Date().getTime();
    passGiven = document.getElementById("password").value;
    if (passGiven.length > 7) {
        if ((passGiven != oldPassGiven) && ((oldRateLimitTimer + 2100) < rateLimitTimer))
            checkPass();
    } else {
        document.getElementById('submit').disabled = true;
...
function clickedScheduler() {
    rateLimitTimer = new Date().getTime();
    passGiven = document.getElementById("password").value;
    if (passGiven.length > 7) {
        if ((oldRateLimitTimer + 2100) < rateLimitTimer) {
            checkPass();
        } else {
            if (clicker = 1) {
                clicker = 0
                setTimeout(clickedScheduler, 2100);
            }
        }
    } else {
        document.getElementById('submit').disabled = true;
...
function checkPass() {
    oldPassGiven = document.getElementById("password").value;
    hashedPass = sha1(oldPassGiven);
    urlString = "https://haveibeenpwned.com/api/v2/pwnedpassword/" + hashedPass;
    xmlHttp.open("GET",urlString,true);
    xmlHttp.send();
    oldRateLimitTimer = new Date().getTime();
    xmlHttp.onloadend = function() {
        if (xmlHttp.status == "404") {
            document.getElementById('password').style.backgroundColor = "springgreen";
